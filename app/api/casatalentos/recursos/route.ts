...
function esRecursoVisibleEnSolapa(
  recurso:
    | {
        slug?: string | null
        tipo?: string | null
  ...
  if (recurso.slug && RECURSOS_OCULTOS_SOLAPA.has(recurso.slug)) return false
...
export async function GET() {
  try {
    ...
    if (!esAdmin) {
      const acceso = await resolveActivityAccess("casatalentos", auth.actor.email)

      return NextResponse.json({
        ok: true,
        recursos: (acceso.recursos || [])
          .filter((item) => esRecursoVisibleEnSolapa(item))
          .map((item) => ({
            id: item.id,
    ...
    return NextResponse.json({
      ...
      recursos: ((data || []) as ActividadRecursoRow[])
        .filter((item) => esRecursoVisibleEnSolapa(tomarRecurso(item)))
...
export async function POST(req: Request) {
  try {
    ...
    const actividad = await asegurarActividadBase("casatalentos")
    const supabase = createAdminSupabaseClient()
    const slugBase = slugify(body.titulo.trim()) || "recurso-casatalentos"
