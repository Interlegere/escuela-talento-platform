...
- **Proyecto In+Posible** (`app/proyecto-inposible/`, `components/proyecto-inposible/`, `lib/proyecto-inposible*.ts`): landing de una campaña puntual, separada del resto de la plataforma — "Programa+Mentoría" de 3 meses (talleres en vivo + sesión 1 a 1 + espacio de Entusiasmento + soporte por WhatsApp), con cierre de inscripción fijo (`FECHA_CIERRE_INSCRIPCION` en `lib/proyecto-inposible.ts`) y fecha de arranque propia. Sistema de diseño propio (paleta tinta/dorado/crema, tipografías Lora + Instrument Sans, tokens en `app/proyecto-inposible/tokens.ts`), sin relación con `app/globals.css`. Tres pantallas: la landing (`page.tsx` — carrusel de proyectos egresados que pasaron por el programa, testimonios, precios, FAQ, banda fija que sigue el scroll con el botón de inscripción), `/gracias` (después de elegir transferencia, con comprobante por WhatsApp) y `/listo` (vuelta desde Mercado Pago — nunca afirma que el pago está acreditado, cualquiera con el link puede abrirla sin haber pagado; lee `status`/`collection_status` con tolerancia a cualquier valor o ninguno). La preinscripción (`/api/preinscripcion`) guarda en base y manda un mail de confirmación con los datos de pago (`lib/mailing.ts`). La tarjeta que se ve al compartir el link (`opengraph-image.tsx`) es **estática** — se genera una sola vez por build con Satori (`ImageResponse`) y se reencodea a JPEG con `sharp` (Satori solo produce PNG, que pesaba ~500KB; el JPEG queda en ~56KB) — cualquier cambio de texto ahí necesita un deploy nuevo antes de volver a compartir el link, porque WhatsApp cachea la primera vista previa que consigue. Decisión tomada y no reabierta: **sin cuenta regresiva** — la página dice explícitamente que no es un curso enlatado, y un reloj en cuenta regresiva es justo el emblema visual de eso. Todo el contenido (testimonios, textos, proyectos del carrusel) llega de a "Prompt N para Claude Code" — texto exacto + checklist de verificación — y se implementa **literal, nunca parafraseado**; cada cambio se verifica con Playwright real contra un build local (capturas a escritorio ancho y a 390px, conteos en el DOM, contraste cuando aplica) antes de pushear, y el deploy de Vercel se sondea con `mcp__claude_ai_Vercel__get_deployment` (nunca `list_deployments`, que devuelve una respuesta enorme) hasta `READY` y se verifica contra producción antes de confirmarle a Nicolás que puede compartir el link.
...
## 4. Pendiente
- ~~No se hizo commit todavía.~~ **RESUELTO**: commiteado y pusheado (`e271a76`) en una sesión posterior no documentada acá.
...
# Sesión de trabajo 2026-09-06 → 2026-09-07 — Proyecto In+Posible: Prompts 30 a 35

## 1. Nota sobre esta entrada
Proyecto In+Posible es un módulo construido a lo largo de muchas sesiones anteriores (al menos "Prompt 1" a "Prompt 29" — landing completa, pantallas de pago, mails transaccionales, imagen de compartir) que nunca se documentó en este archivo, quedó fuera del log por completo hasta ahora. Esta entrada solo cubre lo que pasó en la sesión visible (Prompts 30 a 35); el resto de esa campaña vive en `git log`, no acá. Ver la descripción del módulo más arriba, en "Los módulos, uno por uno".

## 2. Qué se hizo, por prompt
- **Prompt 30** (`38acc43`): Leva y Ser Refugio salen del carrusel de proyectos y de la tira del hero — nunca confirmaron permiso de logo, código y archivos borrados. Entran FADEXA, TadByte y Re.Creo pero comentados en `lib/proyecto-inposible-proyectos.ts` hasta que cada uno confirme el permiso (archivo ya cargado en `public/proyectos/`, una sola línea para descomentar cuando llegue el ok). Verificado que el carrusel (triplicado a propósito para el loop infinito) sigue sin huecos con 5 logos en vez de 7.
- **Prompt 31** (`36a83ca`): FADEXA confirma permiso, entra. Seis ajustes de texto sin lógica nueva: fechas de talleres en líneas separadas (antes una sola línea con puntos medios que se partía mal en mobile); "y científicas" → "incluso de la tecnología" en el párrafo de IA; dos renglones de la tabla de precios reescritos ("con posibilidad de grabación" en vez de prometer un plazo fijo ahí mismo, que ya explican otros dos lugares sin tocar); se saca la referencia rota "abajo vas a leer..." de "El valor diferencial" (los testimonios habían quedado arriba, no abajo, después de un reordenamiento de otra sesión); la frase del CTA de WhatsApp, reescrita.
- **Prompt 32** (`49304ff`): entran TadByte y Re.Creo (permisos confirmados) — quedan 8 proyectos en el carrusel y en la tira del hero. TadByte todavía no tiene sitio: se renderiza sin `<a>` (elemento estático, nunca un link muerto con `href="#"`). Título "Lo dicen ellos" → "Con sus palabras" (los tres testimonios eran de mujeres, "ellos" se leía como descuido).
- **Prompt 33** (`a76ee1d`): la franja dorada de la imagen de compartir decía "EMPIEZA EL 14 · CIERRA EL 11 DE SEPTIEMBRE" sin decir de qué era cada fecha (y se podía leer al revés) — pasa a "INICIO 14/09 · INSCRIPCIONES HASTA EL 11/09". Como esa imagen es estática (se genera una sola vez por build), hubo que regenerarla con un deploy nuevo antes de que Nicolás pudiera compartir el link — verificado 1200×630, `image/jpeg`, bajo 100KB, legible reducida a 400px de ancho (tamaño real de vista previa de WhatsApp).
- **Prompt 34** (`58d6cbd`): la barra fija que sigue el scroll decía "Cierra el viernes 11" sin decir qué cierra — es la frase más visible de toda la página (pegada arriba durante todo el scroll). Pasa a "Inscripción abierta hasta el viernes 11" en escritorio e "Inscripción hasta el 11" en mobile (verificado a 390px de ancho que entra en una sola línea sin apretar el botón).
- **Prompt 35** (`a1f2156` + `f090337`): cuarto testimonio, Julieta — creadora de India Eventos y Arcadia Park, primera en el orden, con los dos logos de sus proyectos lado a lado (campo nuevo `Testimonio.logosProyecto`, un array con alt propio por logo, separado de `logoProyecto` para no tocar el caso de un solo proyecto). Primer commit sin foto (el texto del prompt decía explícitamente que todavía no había, aunque venía una imagen adjunta en el mismo mensaje — se siguió el texto escrito, no la imagen, para no inventar); un mensaje después Nicolás confirmó que esa foto sí iba, así que se agregó `public/testimonios/juli.jpg` y el nombre visible pasó de "Juli" a "Julieta". La grilla de testimonios pasó de 3 a 2 columnas en escritorio (con 4 tarjetas, 3 columnas dejaba una huérfana sola en la fila de abajo).

## 3. Patrón de trabajo de esta campaña (para sesiones futuras)
Cada cambio llega como "Prompt N para Claude Code": texto exacto a usar (nunca parafrasear ni pulir copy — es transcripción, no edición), specs de maquetado, y un checklist de verificación explícito. El flujo, sin excepción: implementar literal → `typecheck` + `lint` → `build` local → verificar con Playwright real contra un `next start` local (capturas a escritorio ancho y a 390px, conteos en el DOM, medición de contraste cuando aplica — nunca confiar solo en el código fuente) → commit (archivos puntuales, nunca `git add -A`) → push a `main` → sondear el deploy de Vercel (`mcp__claude_ai_Vercel__get_deployment` con el id exacto — nunca `list_deployments`, devuelve una respuesta enorme e innecesaria) hasta `READY` → verificar contra producción real → recién ahí confirmarle a Nicolás que puede compartir el link (WhatsApp cachea la primera vista previa que consigue, así que confirmar antes de que él comparta importa de verdad).

## 4. Pendiente
- TadByte sigue sin sitio — cuando lo tenga, sumarle el link (mismo patrón que los demás, una sola línea en `lib/proyecto-inposible-proyectos.ts`).
- Nada más pendiente de este bloque de prompts — el resto de la campaña (precios, FAQ, textos de "Cómo funciona", etc.) está estable desde sesiones anteriores no documentadas acá.

---

# Sesión de trabajo 2026-09-06 → 2026-09-07 (continuación) — Entusiasmento: quién participa de la reunión grupal + reuniones sin sincronizar en Google Calendar

## 1. Objetivo
Con Mentorías compartiendo ahora el espacio de Entusiasmento (sesión 2026-09-01), Nicolás pidió poder distinguir quién participa de la reunión semanal grupal de quién no — mismo caso ya resuelto para el sistema de puntos (`suma_puntos_grupales`, sesión 2026-09-02, pensado para gente que viene de Mentorías: usa las herramientas pero no la reunión).

## 2. Qué se hizo
- **`sql/2026-09-06_entusiasmo_proyectos_participa_reuniones.sql`** (pendiente que Nicolás la corra): columna nueva `entusiasmo_proyectos.participa_reuniones_grupales boolean not null default true` — mismo criterio que `suma_puntos_grupales`, nadie deja de ver algo que ya veía al correr la migración.
- **`app/api/entusiasmo/proyecto/route.ts`**: el `PATCH` que ya guardaba `sumaPuntosGrupales` ahora acepta también `participaReunionesGrupales` (uno, el otro, o los dos juntos en el mismo request). **Bug real encontrado y corregido antes de pushear**: el `.select()` de después del upsert pedía de vuelta las columnas de los dos flags sin condición — como ninguno de los dos call sites del frontend usa esa respuesta (los dos vuelven a pedir el proyecto completo con `GET` después de guardar), alternar el flag YA EXISTENTE (`suma_puntos_grupales`) se hubiera roto apenas se desplegara esto, mientras la columna nueva todavía no existiera en producción. Se cambió el `.select()` a pedir solo `id, participante_email` — inmune a que cualquiera de las dos columnas todavía no exista.
- **`app/casatalentos/page.tsx`**: el bloque de "Reunión semanal" (próximo encuentro + botón de Meet) ahora se oculta para quien tiene `participa_reuniones_grupales: false` — admin lo sigue viendo siempre, para cualquiera que esté mirando. Checkbox nuevo, admin-only, en la solapa de cada participante: "{nombre} participa de la reunión grupal" (mismo patrón visual y de guardado que el checkbox de puntos ya existente).
- **Limpieza en Google Calendar**: Nicolás reportó que no tenía sincronizadas las reuniones de Entusiasmento y pidió borrar las que hubiera para recargarlas a mano. Investigado antes de tocar nada (vía Supabase MCP, de solo lectura primero): no había ninguna reunión activa cargada para septiembre — la última que sí se sincronizó de verdad fue el lunes 31/08 (serie con `google_event_id` real, `sync_status: sincronizado`). Lo único que "existía" para septiembre/octubre eran 7 filas fantasma (`disponibilidades` ids 30 a 36), ya canceladas, sin `google_event_id`, sin ninguna `reserva` que las referenciara — restos de un patrón viejo (viernes 07:00) de antes de que la reunión pasara a los lunes 19:30. Se borraron. **No hay ningún cron que extienda sola la serie recurrente de reuniones** (a diferencia de las tareas semanales de Entusiasmento, que sí se auto-extienden vía `completarHorizonteDeSeries`) — la reunión se cargó en su momento como un lote fijo de N semanas desde `/agenda` ("Repetir semanalmente") y ahí se frenó. Nicolás tiene que volver a cargar la reunión de septiembre desde ahí, como ya lo tenía previsto — debería sincronizar a Google Calendar igual que lo hizo de mayo a agosto.

## 3. Verificado en vivo
Login real como admin contra la base de producción (vía `npm run dev` — `next start` corre con `NODE_ENV=production`, que deshabilita los usuarios de prueba hardcodeados de `lib/auth.ts`). Confirmado por API, contra un participante real:
- Sin la columna nueva corrida: `PATCH` con `sumaPuntosGrupales` solo sigue funcionando exactamente igual que antes (la regresión se evitó); `PATCH` con `participaReunionesGrupales` falla con un 500 estructurado, no rompe nada más.
- El checkbox nuevo aparece correctamente en la solapa de un participante real, con el label esperado, y el bloque de "Reunión semanal" se sigue mostrando (con la columna ausente, `!== false` lo trata como visible por default — mismo criterio que el resto de estos flags en el proyecto).

`typecheck`/`lint` limpios, mismo baseline de 24 problemas preexistentes, sin warnings nuevos.

## 4. Pendiente
- **Nicolás tiene que correr `sql/2026-09-06_entusiasmo_proyectos_participa_reuniones.sql`** antes de que el checkbox nuevo guarde de verdad (hasta entonces, tocarlo muestra un mensaje de error, no rompe nada más).
- Nicolás tiene que recargar la reunión semanal de Entusiasmento de septiembre desde `/agenda` → "Repetir semanalmente".
- **CoFruto en escritorio necesita una pasada de UX** (reportado por Nicolás, 2026-09-07): la vista está pensada y se ve bien en celular, pero en pantallas grandes puede mejorar — todavía sin diagnóstico ni diseño concreto, queda para una sesión futura dedicada a mirarlo en detalle.
